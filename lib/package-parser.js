...
packageParser.matchDependencies = function matchDependencies(path_to_pkg) {
  ...
  * Response is an object that shows the docker module, and the npm modules that
  * need it. This response is in the form of { docker: [npm, npm, ..., npm] }.
  * Example response:
  ...
  **/
  ...
  return new Promise((res, rej) => {
    ...
    // }).then(() => {
    //   dockerPromises = this.fetchDockers();
    }).then(() => {
      // When all arrays have resolved
      console.log('dockerPromises', dockerPromises);
      console.log('depKeywords', depKeywords);
    ...
    // Promise.then with fetchNPM to get our object of matches and return that
}
